...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let num_samples = data.src_tensors.len();
    ...
    for epoch in 1..=epochs {
        ...
        while i < num_samples {
            ...
            let batch_src = Tensor::stack(&data.src_tensors[i..end], 0).to_device(device);
            let batch_trg = Tensor::stack(&data.trg_tensors[i..end], 0).to_device(device);
            opt.zero_grad();
